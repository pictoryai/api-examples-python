...
def create_highlights(token,transcriptdata):
    BASE_URL=os.getenv('BASE_URL')
    HIGHLIGHTS_ROUTE=os.getenv('HIGHLIGHTS_ROUTE')
    WEBHOOK_URL=os.getenv('WEBHOOK_URL')
    USER_ID = os.getenv('USER_ID')
    url=BASE_URL+HIGHLIGHTS_ROUTE
    renderequestpayload=payloads.create_highlights_payload(transcriptdata['transcript'],60,WEBHOOK_URL)
    headers=payloads.set_headers(token,USER_ID)
    ...
    try:
            ...
            print(f"Error while higlights: {e}")
            return None

#Waits for highlighgts job to get complete
def wait_for_highlights_job_to_complete(token,jobid):
...
def main():
    ...
    jobid=create_highlights(token,transcriptiondata)
    highlightdata=wait_for_highlights_job_to_complete(token,jobid)
    print(highlightdata)
